...
Most existing approaches flatten a molecular graph into a linear sequence of atoms and bonds, then train a language model to generate these sequences token by token. This is like describing a building by listing every brick in order — it works, but it throws away the architectural blueprint. The model has to rediscover structural patterns (like "this is a six-membered ring") purely from raw token statistics.

MOSAIC takes a different approach. Instead of generating molecules atom-by-atom, it first learns a hierarchical decomposition — grouping atoms into meaningful clusters, and clusters into larger motifs — then generates molecules top-down, from coarse structure to fine detail. Think of it as teaching the model the blueprint before asking it to place the bricks.
